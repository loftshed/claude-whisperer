...
test('stop blocks while the handoff is missing and yields once it exists', (ctx) => {
  ...
  assert(allowed.continue === false, 'once the handoff exists stop must end the session, overriding other stop hooks');
  assert(!('decision' in allowed), 'stop must not block once the handoff exists');
  ...
  assert(again.continue === false, 'a later stop in the same window still ends the session (a /goal loop)');
});
...
test('a new quota window lifts the post-bailout halt', (ctx) => {
  ...
  writeFileSync(paths(ctx).handoff, '# handoff\n');
  stopHook(ctx, hookPayload(ctx));
  sample(ctx, { percent: 5, resets: 2000018000 });
  assert(stopHook(ctx, hookPayload(ctx)).stdout.trim() === '', 'after the window resets stop is silent again');
});
...
test('stop blocks are bounded', (ctx) => {
  ...
  assert(third.continue === false && !('decision' in third), 'the third stop must end the session instead of blocking');
});
...
test('INTEGRATION: a continuing task crosses the threshold, hands off, and yields', (ctx) => {
  ...
  assert(ended.continue === false && !('decision' in ended), 'the turn should end, not be blocked');
  ...
  assert(!stopHook(ctx, hookPayload(ctx)).stdout.includes('"block"'), 'no further stop blocks after the handoff');
