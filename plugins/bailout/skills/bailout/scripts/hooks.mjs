...
async function modeStop(config) {
  ...
  if (state.stage === 'bailout-done') {
    writeState(state);
    halt(`Usage bailout: handoff written to ${state.handoffs.bailout}. Resume from it after the five-hour window resets.`);
    return;
  }
  ...
  if (state.stopBlocks >= config.maxStopBlocks) {
    writeState(state);
    halt('Usage bailout: the handoff was requested but not written; the session-end hook will save a recovery snapshot.');
    return;
  }
  const now = nowSeconds();
  state.stopBlocks += 1;
